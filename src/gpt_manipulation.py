...
if __name__ == "__main__":
    ...
    print("the need obj idx is:", need_obj_idx)
    ...
    CONFIG_PATH = "./task_config/{}.yaml".format(task_name)
    SCALE = 0.3
    LEN_OBJ = 1
    NUM_ENV = 1
    ...
    for object_idx, name in enumerate(train_assets_keys):
        ...
        object_name = name
        all_assets.append({"name": object_name, "val":val})
    ...
    # use gpt to get target pose
    def tensor_to_tuple(tensor):
        return tuple(tensor.detach().cpu().numpy().squeeze())
    from prompt_tool import Agent, Prompt, ManiPrompt,ManiParser

    config_path = "prompt_config/mani.json"
    example_path = "prompt_config/mani_example.json" 
    prompt = ManiPrompt(config_path,example_path,task = task_name)
    env_info = save_json["scene"]
    prompt.append_environment_info(env_info)
    prompt.submit()
    print(prompt.get_content()[1]["content"])


    parser = ManiParser()
    agent = Agent(parser)
    error, actions = agent.request(prompt)
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        if t >= FRANKA_UPDATE_TIME:
            ...
            if grasp_time > 0:
                ...
                if temp_action_idx >= len(actions):
                    continue
                elif actions[temp_action_idx]['type'] == "grasp":
                    exec_grasp_flag = torch.ones((1,1),dtype = bool,device="cuda:0")
                    grasp_time = 5
                elif actions[temp_action_idx]['type'] == "release":
                    exec_grasp_flag = torch.zeros((1,1),dtype = bool,device="cuda:0")
                    grasp_time = 5
                elif actions[temp_action_idx]['type'] == "move":
                    waypoint_pos = np.array(actions[temp_action_idx]['target'])
                    waypoint_quat = ee_quat[0]
                    waypoint = np.concatenate((waypoint_pos,waypoint_quat))
                    print("the waypoints is:", waypoint)
                    planned_time, planned_joints_pos, planned_joints_vel = plan_traj(dofs_pos,waypoint,motion_gen)
                elif actions[temp_action_idx]['type'] == "rotate_anti_clockwise":
                    hand_q = ee_quat[0].cpu().numpy()
                    ...
                    target_p = ee_pos[0].cpu().numpy()
                    
                    waypoint = np.concatenate((target_p,target_q))
                    planned_time, planned_joints_pos, planned_joints_vel = plan_traj(dofs_pos,waypoint,motion_gen)
                elif actions[temp_action_idx]['type'] == "rotate_clockwise":
