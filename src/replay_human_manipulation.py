...
if __name__ == "__main__":
    ...
    {"name": "--demo_path", "type": str, "default" : ""},
    {"name": "--demo_dir", "type": str, "default" : "human_demonstration"}]
    ...
    path_list = args.demo_path.split("_")
    task_name = "_".join(path_list[:-2])
    need_obj_idx = int(path_list[-1])
    ...
    demo_path = os.path.join(args.demo_dir,args.demo_path + ".json")

    with open(demo_path, "r") as f:
        demo_data = json.load(f)
        actions = demo_data["scene"]["actions"]
        ...
        if t >= FRANKA_UPDATE_TIME:
            ...
            if grasp_time > 0:
                ...
                if temp_action_idx >= len(actions):
                    continue
                elif actions[temp_action_idx] == "GRASP":
                    exec_grasp_flag = torch.ones((1,1),dtype = bool,device="cuda:0")
                    grasp_time = 5
                elif actions[temp_action_idx] == "RELEASE":
                    exec_grasp_flag = torch.zeros((1,1),dtype = bool,device="cuda:0")
                    grasp_time = 5
                else:
                    waypoint = np.array(actions[temp_action_idx])
